...
function solventparameters(s = solvent)
    ...
    elseif s == "xenon" # Some values for anion and cation generic env.
        if charge < 0 # Anion
            𝜀 = @isdefined(dielectric) ? dielectric : 1.544^2
            return (𝜀, 2.0, 58.44, 16, 2.49)
        else # Cation
            𝜀 = @isdefined(dielectric) ? dielectric : 1.544^2
            return (𝜀, 40.0, 58.44, 16, 2.49)
        end
        #𝜀 = @isdefined(dielectric) ? dielectric : 1.6403^2 # CsCl compression chamber
...
function atomicradii(type = radiustype)
    ...
    elseif type == "ionic" # ionic radii from 1e-3 electron density cutoff.

        radii_plus_1 = Dict( # Charge 1
        #2s
        "Li"=> 0.98,    "3" => 0.98, # Li+
        "Be"=> 1.81,    "4" => 1.81, # Be+
        #2p
        ...
        radii_plus_3 = Dict( # Charge 3
        #2p
        "N" => 1.28,    "7" => 1.28, # N3+
        )

        radii_minus_3 = Dict( # Charge -3
        #2p
        "N" => 3.40,    "7" => 3.40, # N3-
        )
        ...
        if charge == 1
            return radii_plus_1 
        elseif charge == -1
            return radii_minus_1
        elseif charge == 2
            return radii_plus_2
        elseif charge == -2
            return radii_minus_2
        elseif charge == 3
            return radii_plus_3
        elseif charge == -3
            return radii_minus_3
        else
            error("charge not supported. Try 1, -1, 2, -2, 3, or -3")
        end
...
function gjfvc(geom = geometries, 𝑓 = scalingfactors)
    ...
    num_tess = round((4π*(first(𝑓)*𝑟ₐ[atoms[1]])^2)/tesserae)
...
# The Pauli repulsion energy is found in the end of the SCF cycle as "Quantum repulsion energy".
# Extract Pauli repulsion energy from gaussian output files at every scaling factor 𝑓.
function get_𝐸ₚₐᵤₗᵢ(𝑓 = scalingfactors)
    a = length(𝑓)
    𝐸ₚₐᵤₗᵢ = Array{Float64}(undef, a)
    j = 1   # j ranges from 1 to total amount of scaling factors
    open("Ger.log", "r") do file
        for line in eachline(file)
            # Search for final print of SCF cycle of Pauli repulsion energy.
            if occursin("Quantum repulsion energy                 (kcal/mol)", line)
                𝐸ₚₐᵤₗᵢ[j] = parse(Float64, split(line)[end]) / 627.503 # 1 hartree = 627.503 kcal/mol
                j += 1    
            end
        end
    end
...
function get_s_square(𝑓 = scalingfactors)
    a = length(𝑓)
    𝑠² = Array{Float64}(undef, a)
    j = 0   # j ranges from 0 to total amount of scaling factors. Starts at 0 because "SCF Done" comes before "<S**2>"
    open("Ger.log", "r") do file
        for line in eachline(file)
            # Search for final print of SCF cycle of Pauli repulsion energy.
            if occursin("<S**2>", line)
                if j == 0
                    𝑠²[1] = parse(Float64, split(line)[end-2]) 
                    # However, the initial guess still should not write in index 0...
                else
                    𝑠²[j] = parse(Float64, split(line)[end-2]) 
                    # TODO: Possible OBOB due to minus signs. <S**2>= 2.00 vs <S**2>=-2.00
                end
                # Only increment j when SCF cycle is finished.
...
function writeproperties3(𝑉𝑐 = 𝑉𝑐, 𝑓 = scalingfactors)
    ...
    open(filename_without_extension * "properties.dat", "w") do file
        ...
        write(file, "#      𝑓         𝑉𝑐      𝑠       𝜀      𝜌ₛₒₗ        𝒵   𝐸(nu-ch)   𝑑𝐸(nu-ch)╱𝑑𝑠   𝐸(ch-ch)   𝑑𝐸(ch-ch)╱𝑑𝑠   𝐸(el-ch)   𝑑𝐸(el-ch)╱𝑑𝑠     𝐸ₚₒₗₐᵣ   𝑑𝐸ₚₒₗₐᵣ╱𝑑𝑠     𝐸ₚₐᵤₗᵢ   𝑑𝐸ₚₐᵤₗᵢ╱𝑑𝑠           𝐺ₑᵣ       𝑝ₐ       𝑝ₙ      𝑝ₙ′     𝑉/𝑉₀     S^2\n")
        write(file, "#                Å³                     g/ml                  Eₕ             Eₕ         Eₕ             Eₕ         Eₕ             Eₕ         Eₕ           Eₕ         Eₕ           Eₕ            Eₕ      GPa      GPa      GPa     1     1\n")
        for j in 1:a
            @printf(file, "%-3d  %5.3f  %7.3f  %5.3f  %7.3f  %7.3f  %7.4f  %9.6f  %13.6f  %9.6f  %13.6f  %9.6f  %13.6f  %9.6f  %11.6f  %9.6f  %11.6f  %12.6f  %7.3f  %7.3f  %7.3f  %7.3f  %7.3f\n", 
                            j, 𝑓[j], 𝑉𝑐[j], 𝑠[j], 𝜀[j], 𝜌[j], 𝒵[j], 𝐸_nuclei_charges[j], 𝑑𝐸_nuclei_charges╱𝑑𝑠[j], 𝐸_charges_charges[j], 𝑑𝐸_charges_charges╱𝑑𝑠[j], 𝐸_electrons_charges[j], 𝑑𝐸_electrons_charges╱𝑑𝑠[j], 𝑊ₚₒₗ′[j], 𝑑𝑊ₚₒₗ′╱𝑑𝑠[j], 𝐸ₚₐᵤₗᵢ[j], 𝑑𝐸ᵣ╱𝑑𝑠[j], 𝐺ₑᵣ[j], 𝑝ₐ[j], 𝑝ₙ[j], 𝑝ₙ′[j], 𝑉𝑐[j]/𝑉𝑐[1], 𝑠²[j])
        end
        write(file, "\n")
...
function calc_𝒵_new_pointcharges(𝒵, 𝑅𝑟𝑒𝑓, 𝑓=[scalingfactors[1]])
    ...
    if numerator < 0
        numerator = 0.9 * abs(numerator) # Still decreasing but not crossing zero
    end
    𝒵_new =  numerator / denominator

    open(filename_without_extension * "Z.dat", "a") do file # IterativeZ
#=         println(file, 
    ...
    #     end
         ...
         if model == "pointcharges"
            ...
            if impose_equilibrium == true # at 1st scalingfactor so that p(f0)=0
                open(filename_without_extension * "Z.dat", "w") do file 
                    @printf(file, "       𝜌_sol            𝒵      𝐸_nu_ch      𝐸_ch_ch      𝐸_el_ch       𝐸ₚₐᵤₗᵢ          𝑛𝑡𝑠      𝑒𝑓𝑔/𝑛𝑡𝑠          𝑒𝑓𝑔           𝐼₁           𝐼₂    numerator  denominator        𝒵_new\n",)
                ...
                while !(0.9999 < 𝒵_new/𝒵_old < 1.0001) # self-consistent calculation of 𝒵
                    global iteration += 1
                    if iteration > 10 # 10 is already generous.
                        break
                    end
            ...
            # numerical pressure
            𝑝ₙ = calc_numerical𝑝(𝑉𝑐, 𝐸ₜₒₜ)  # by Murnaghan EOS fitting
