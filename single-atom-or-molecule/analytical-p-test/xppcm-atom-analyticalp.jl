...
function gjfgeranalytical(geom = geometries, 𝑓 = scalingfactors)
    ...
    for j in 1:a
        ...
        open("Ger.gjf", "$(j == 1 ? "w" : "a")") do file
            ...
            # New line--number of tesserae, 1 is the keyword, input fortran file extension 
            # corresponding to the scaling factor, output fortran file extension (+1)
            write(file, "$n,1,$(Int(round(𝑓[j]*1000))),$(Int(round(𝑓[j]*1000+1)))\n\n")
        ...
        # Write necessary fortran input file
        # Only work on single atom. Passing the scaling factor and the atomic radius
        writefortinput(𝑓[j], last(𝑓), 𝑟ₐ[atoms[1]])
    end
end


function writefortinput(𝑓, last𝑓, 𝑟ₐ)
    c = gettesseraecoordinates()
    writedlm("fort.$(Int(round(𝑓*1000)))", c * 𝑟ₐ * 𝑓 / last𝑓)
end
...
# extract the total number of tesserae from tesserae.off
function getnumberoftesserae()
    n = 0  # initiating the variable
    open("tesserae.off", "r") do file
        readline(file)
        secondline=readline(file)
        # the 2nd number on the 2nd line is the number of tesserae
        n = parse(Int64, split(secondline)[2])
    end
    return n
end
...
# from tesserae.off from the last Vc.gjf calculation; last(scalingfactors)
function gettesseraecoordinates()
    n = getnumberoftesserae()
    tesseraecoordinates = Array{Float64}(undef, n,3)  # n*3 2D array
    x1 = x2 = x3 = y1 = y2 = y3 = z1 = z2 = z3 = 0
    linecount = 1
    i = 1   # i ranges from 1:n
    open("tesserae.off", "r") do file
        for line in eachline(file)
            if linecount >= 3 && linecount <= 3 * n + 2
                if linecount % 3 == 0
    ...
    return tesseraecoordinates
end


# extract electric field gradient data from fort.xxxx file
function getEFG(𝑓 = scalingfactors)
    a = length(𝑓)
    efg = zeros(a)
    for j in 1:a
        linecount = 1
        open("fort.$(Int(round(𝑓[j]*1000))+1)", "r") do file
            for line in eachline(file)
                if linecount % 4 == 2
                    #zz = parse(Float64, split(line)[4])
                    efg[j] += parse(Float64, split(line)[4])
                elseif linecount % 4 == 3
                    #(yy, xx) = [parse(Float64, s) for s in split(line)[1:2]]
                    efg[j] += sum([parse(Float64, s) for s in split(line)[1:2]])
                end
                linecount += 1 
            end
        end
