...
function write_properties(𝑓::NTuple{7, Float64}, 
    ...
    open("properties.dat", "w") do file
        for i in 1:nos
            ...
            println(file, "#    𝑓         𝑉𝑐(𝑓)      𝑠(𝑓)        𝑠̄(𝑓)        𝜀(𝑠̄)        𝜌(𝑠̄)        𝑉ₘ(𝑠̄)       𝐸𝑔𝑎𝑠         𝐸ₑₗₑₛₜₐₜ  𝐸ₚₐᵤₗᵢ        𝐺𝑒𝑟             𝑝        𝑝̄        𝑉𝑐𝑎𝑣       𝑝̄𝑉𝑐𝑎𝑣         𝐸𝑐𝑎𝑣          𝐺𝑐𝑎𝑣         𝐺𝑡𝑜𝑡             Δ𝐺𝑡𝑜𝑡")
            println(file, "               Å³                                                                     Eₕ           kcal/mol  kcal/mol      Eₕ              GPa      GPa      Å³          Eₕ            Eₕ            Eₕ           Eₕ            kcal/mol")
            for j in 1:nosf
                pv = 𝑝̄[j] * 𝑉𝑐𝑎𝑣[i,j] * 2.293712569e-4
                @printf(file, "%d    %.3f    %7.3f    %.6f    %.6f    %.6f    %.6f    %7.3f    %.8f    %6.2f    %6.2f      %.8f    %6.3f    %6.3f    %7.3f    %.8f    %.8f    %.8f    %.8f    %7.2f\n", 
