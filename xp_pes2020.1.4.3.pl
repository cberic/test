...
#                   f0 f1 ... fn \
#                   solvent  eta cavity fcav tsare molec mrng cavmd  
#
...
#
# 4) 'solvent': 
#    'solvent' is a string of characters that specifies the external non-polar medium.
...
# 
# 10) mrng:
#     mrng = 'math' Murnaghan fitting with Mathematica
#           = 'py'   Murnaghan fitting with Python
# 11) cavmd:
#     cavmd = 'hs' Cavitation energy with hard spheres approximation
#           = 'ss' Cavitation energy with soft spheres approximation
#  
# Example:                     
#        xp_pes20v1  3 React.com  TS.com   Prod.com    \
#                    1.2 1.15 1.10 1.05 1.00 0.975  0.95  \
#                    C6H12  3  vdw 1.2  0.075 0 math &
#     
...
$cavmd=$ARGV[$npointer]; $npointer++;
if($cavmd =~ /hs/){print 'cavmod=hard spheres',"\n"};
if($cavmd =~ /ss/){print 'cavmod=soft spheres',"\n"};
#
...
#----------------------------------------------------
for($k=1; $k<=$n_struct;$k++)
   {$infile = $ARGV[$k]; @array1=split(/\./,$infile);
    $ingas = $array1[0]."_gas.com";
    $chkgas = $array1[0]."_gas.chk";
    $outfile = $array1[0].'_gas.log'; 
#
    open (FL,$ARGV[$k]);
    open (FPCM,">$ingas"); 
    print FPCM "%chk=".$chkgas,"\n";
    while ($line = <FL>) 
          {if(!($line =~ /#/))
             {print FPCM $line}
           if($line =~ /#/) 
             {chop($line);$newline=$line.' 6d 10f'; print FPCM $newline, "\n"}           
           }
#
    my $cmd = 'g16 '.$ingas;
    system($cmd);
    ...
    while ($line = <FL>) 
          {if($line =~ /SCF Done/) 
             {chop($line);@simpson=split(/\s+/,$line);
              $e0[$k]=$simpson[5]; print $e0[$k]," Hartree\n";
             }
           }
   };
#----------------------------------------------------
#
#  pcm calulations at 1 bar 
#
#----------------------------------------------------
for($k=1; $k<=$n_struct;$k++){
#
...
#@array1=split(/\./,$oldfile);
    $infile = $ARGV[$k]; @array1=split(/\./,$infile);
    $inpcm = $array1[0]."_pcm.com";
    $chkpcm = $array1[0]."_pcm.chk";
    $outpcm = $array1[0].'_pcm.log'; 
...
while ($line = <OF>) {
  if($line=~ /\S/) 
    {if(!($line=~/chk/)){print SCR $line};
      if($line =~ /#/) {print SCR "# scrf=(iefpcm,solvent=",$solvent,",read) nosym 6d 10f \n"};
...
#           if ( $atoms[$n_atoms] =~  /Cl/) {$radius[$n_atoms] = 1.75};
       ...
       {print SCR $line;$n_blanks++;
        if($n_blanks == 3){$n_sfe=$n_atoms;
                           print SCR "norep pcmdoc geomview  nodis nocav g03defaults tsare=",$tsare," \n";
    ...
    while ($line = <FL>) 
          {if($line =~ /SCF Done/) 
             {chop($line);@simpson=split(/\s+/,$line);
              $g0[$k]=$simpson[5]; print $g0[$k]," Hartree\n";
             }
           }
}
#----------------------------------------------------
#
# Template input file for xp-pcm 
#
#----------------------------------------------------
for($k=1; $k<=$n_struct;$k++){
#
...
$n_fact = 0;  
 while($line = <FL> ) {
              if($line =~ / GePol: Cavity volume/){$n_fact++;
                ...
                @simpson = split(/\s+/,$line);
                $volume[$n_fact] = $simpson[5]};
              if($line =~ /PCM non-electrostatic energy/){6
                chop($line);
                @simpson = split(/\s+/,$line);$ecav[$n_fact] = $simpson[5];
                $pV[$n_fact]=$p[$n_fact]*$volume[$n_fact]*(1.0/$cal2J)*$avogadro;
#RC160720
#                if ($cavmd  =~ /ss/) {$ecav[$n_fact]=$ecav[1]; $volume[$n_fact]= $volume[$n_fact]*$s_mat[$k][$n_fact]**3;$pV[$n_fact]=$pV[$n_fact]*$s_mat[$k][$n_fact]**3;
#                                      $gcav[$n_fact]=$ecav[1]*$Eh2kcal_mol+$pV[$n_fact]};
                if ($cavmd  =~ /ss/) {$ecav[$n_fact]=$ecav[1]; $volume[$n_fact]= $volume[$n_fact]*$s[$n_fact]**3;$pV[$n_fact]=$pV[$n_fact]*$s[$n_fact]**3;
                                      $gcav[$n_fact]=$ecav[1]*$Eh2kcal_mol+$pV[$n_fact]};
                if ($cavmd  =~ /hs/) {$gcav[$n_fact]=$ecav[$n_fact]*$Eh2kcal_mol+$pV[$n_fact]};
#RC160720
...
#RC080720
print OUT "\nTable Ib: 
Electronic energy (G_), cavitation energy (G_cav), and total (G_tot) 
energies at p=1bar \n";
printf OUT " %11s %10s %8s %10s \r\n", 'System   ','G_er   ','G_cav  ','G_tot  ';
printf OUT " %11s %10s %8s %10s \r\n", ' ', '[Eh]   ','[Eh]   ','[Eh]   ';
for($k=1; $k<=$n_struct;$k++)
   { @array1=split(/\./,$ARGV[$k]);
     printf OUT "%11s %10.4f %8.4f %10.4f \r\n", $array1[0], $g0[$k], $g0c[$k], $g0t[$k]
   };
print OUT "\n";
#RC080720
if($nmolec == 0){
...
#RC080720
for($j=1; $j<=$n_struct;$j++){
    $dg0t[$j] = ($g0t[$j]-$g0t[$n_ref])*$Eh2kcal_mol;
    $dg0c[$j] = ($g0c[$j]-$g0c[$n_ref])*$Eh2kcal_mol;
    $dg0[$j]  = ($g0[$j]-$g0[$n_ref])*$Eh2kcal_mol;
                               };
#RC080720
#
...
#
# Reference pressure, p0,  for the analysis of the effect of the pressure
# DDG_tot(x;p)= DG_(tot)(x;p)-DG_(tot)(x;p=0GPa)
##RC080720
#  $n_p0 = 1;
#
for($i=1; $i<=$i_fact;$i++){ 
  for($j=2; $j<=$n_struct;$j++){
    $ddgtot[$i][$j] = $dgtot[$i][$j]-$dg0t[$j];
    $ddgcav[$i][$j] = $dgcav[$i][$j]-$dg0c[$j];
    $ddger[$i][$j] =  $dger[$i][$j]-$dg0[$j];
...
printf OUT " %8s %18s %18s %18s\r\n", 'Structure','DG_tot[kcal/mol] ','DG_er[kcal/mol] ','DG_cav[kcal/mol] ';
  for($j=1; $j<=$n_struct;$j++){@array1=split(/\./,$ARGV[$j]);
  printf OUT "%8s %18.1f %18.1f %18.1f \r\n", $array1[0],$dg0t[$j],$dg0[$j],$dg0c[$j];
                               };
#RC080720
...
DDG_tot({i},p)=DG_tot({i},p)-DG_tot({i},p=0GPa";
#printf  OUT "%4.1f", $p[1];
#print OUT "GPa)
print OUT "DDG_er and DDG_cav are the electronic and cavitation contributions, respectively.\n\n";
...
#RC080720
    ...
    $dg0[2]  = 0.0;
    for($j=3; $j<=$n_struct;$j++){
    $dg0t[$j] = ($g0t[$j]-($g0t[$n_ref]+$g0t[$n_ref+1]))*$Eh2kcal_mol;
    $dg0c[$j] = ($g0c[$j]-($g0c[$n_ref]+$g0c[$n_ref+1]))*$Eh2kcal_mol;
    $dg0[$j]  = ($g0[$j]-($g0[$n_ref]+$g0[$n_ref+1]))*$Eh2kcal_mol;
                                };
#RC080720
#
#
...
printf OUT " %8s %18s %18s %18s\r\n", 'Structure','DG_tot[kcal/mol] ','DG_er[kcal/mol] ','DG_cav[kcal/mol] ';
  ...
  printf OUT "%8s %18.1f %18.1f %18.1f \r\n", $string,$dg0t[2],$dg0[2],$dg0c[2];
  for($j=3; $j<=$n_struct;$j++){@array1=split(/\./,$ARGV[$j]);
  printf OUT "%8s %18.1f %18.1f %18.1f \r\n", $array1[0],$dg0t[$j],$dg0[$j],$dg0c[$j];
                               };
#RC080720
for($i=1; $i<=$i_fact;$i++){ print OUT "p[GPa]= "; printf  OUT "%6.1f \n", $p[$i];
